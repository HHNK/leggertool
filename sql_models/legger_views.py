...
def create_legger_view_export_damo(session: sqlite3.Connection):
    ...
    SELECT
        ...
        grondsoort,
        CAST(round(streefpeil, 6)  AS DOUBLE) AS streefpeil,
        CAST(round(zomerpeil, 6)  AS DOUBLE) AS zomerpeil,
        CAST(round(breedte, 8)  AS DOUBLE) AS waterbreedte_BGT,
        CAST(round(debiet_inlaat, 8) AS DOUBLE) AS WS_AANVOERDEBIET,
        CAST(round(debiet, 8)  AS DOUBLE) AS WS_AFVOERDEBIET,
        CAST(round(geselecteerde_bodembreedte, 6)  AS DOUBLE) AS WS_BODEMBREEDTE,
        CAST(round(geselecteerde_diepte, 6)  AS DOUBLE) AS geselecteerde_diepte,
        CAST(round(geselecteerd_waterbreedte , 6)  AS DOUBLE) AS geselecteerde_waterbreedte,
